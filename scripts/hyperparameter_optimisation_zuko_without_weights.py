...
The spectra are not noised (the original ideal spectra).
...
print(
    f"Complete Zuko Flow (7D) trained on true inputs (without weights)"
)
...
def objective(trial):
    ...
    num_transforms = trial.suggest_int("num_transforms", 1, 20)
    ...
    hidden_layers_spline_context = trial.suggest_int(
        "hidden_layers_spline_context", 1, 5
    )
