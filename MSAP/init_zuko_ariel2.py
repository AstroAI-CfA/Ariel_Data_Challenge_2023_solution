"""
This script is used as a base for the plotting scripts. It differs from v1 because the transform of the input parameters
dataset (GTtransform) are independent from the transforms on the tracedata dataset.
...
config['data']['run-id']="7"
config["data"]["n-test"]=1000
...
GTtransform_tg = ComposedTransforms(
    [GTtransform_tg, normalisation_specific_transform(GTtransform_tg(GTtargets))]
) # we normalise every parameter between 0 and 1
GTtransform_tg1 = IndexTransform(
    [0], ScalarTransform(GTaux[:, -1], 1.0)
)  # we substract the estimated radius of the planet to the target radius to only predict the offset
GTtransform_tg1 = ComposedTransforms(
    [GTtransform_tg1, transform_tg.transforms_list[1]]
) 

GTtransform_au = normalisation_specific_transform(
    GTaux
)  # We normalise every auxiliary variable btw 0 and 1

GTtemptransform_ft = ComposedTransforms([Normalise_each(ft="mean_std")]) # we normalise the spectrum by substracting its mean and dividing by its std. We concatenate the mean and std to the features tensor to not lose the information
GTtempft = GTtemptransform_ft(
    GTfeatures.clone()
)  # We apply temporarly the transformation to later normalise btw 0 and 1 the min and max (index=0 and 1)
GTtransform_ft = ComposedTransforms(
    [
        GTtemptransform_ft,
        IndexTransform([0, 1], normalisation_specific_transform(GTtempft[:, 0:2])),
    ]
)  # We normalise the mean and std features between 0 and 1. The spectrum is already normalized.

del GTtempft
del GTtemptransform_ft
gc.collect()

GTtransform_no = normalisation_specific_transform(
    GTnoises
)  # We normalise the uncertainty btw 0 and 1
...
_GTfeatures1 = transform_ft(GTfeatures)
_GTtargets1 = GTtransform_tg1(GTtargets)
_GTaux1 = transform_au(GTaux)
_GTnoises1 = transform_no(GTnoises)


GTft = torch.cat([_GTaux, _GTfeatures, _GTnoises], dim=1)
GTft1 = torch.cat([_GTaux1, _GTfeatures1, _GTnoises1], dim=1)
...
GTtest_features1 = GTft1[-4000:, :]
GTtest_spectra1 = _GTfeatures1[-4000:, :]
GTtest_targets1 = _GTtargets1[-4000:, :]
GTtest_noises1 = _GTnoises1[-4000:, :]
GTtest_aux1 = _GTaux1[-4000:, :]
...
del _GTfeatures, GTft, _GTtargets, _GTnoises, _GTaux
del _GTfeatures1, GTft1, _GTtargets1, _GTnoises1, _GTaux1
gc.collect()